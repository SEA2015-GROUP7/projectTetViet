...
public class SecurePreferences {
	 ...
	 */
	public static SecurePreferences getInstance(Context context, String preferenceName, String secureKey, boolean encryptKeys) throws SecurePreferencesException {
		if(mSecurePreferences == null){
			mSecurePreferences = new SecurePreferences();
		}
		try {
			mSecurePreferences.writer = Cipher.getInstance(TRANSFORMATION);
			mSecurePreferences.reader = Cipher.getInstance(TRANSFORMATION);
			mSecurePreferences.keyWriter = Cipher.getInstance(KEY_TRANSFORMATION);

			initCiphers(secureKey);

			mSecurePreferences.preferences = context.getSharedPreferences(preferenceName, Context.MODE_PRIVATE);

			mSecurePreferences.encryptKeys = encryptKeys;
		}
	...
	protected static void initCiphers(String secureKey) throws UnsupportedEncodingException, NoSuchAlgorithmException, InvalidKeyException,
			InvalidAlgorithmParameterException {
		IvParameterSpec ivSpec = getIv();
		SecretKeySpec secretKey = getSecretKey(secureKey);

		mSecurePreferences.writer.init(Cipher.ENCRYPT_MODE, secretKey, ivSpec);
		mSecurePreferences.reader.init(Cipher.DECRYPT_MODE, secretKey, ivSpec);
		mSecurePreferences.keyWriter.init(Cipher.ENCRYPT_MODE, secretKey);
	}
	
	protected static IvParameterSpec getIv() {
		byte[] iv = new byte[mSecurePreferences.writer.getBlockSize()];
		System.arraycopy("fldsjfodasjifudslfjdsaofshaufihadsf".getBytes(), 0, iv, 0, mSecurePreferences.writer.getBlockSize());
